...
class OpenCloseLob
{
    ...
    throws SQLException
  {
    ...
    while (rset.next ())
    {
     OracleBlob blob = (OracleBlob) rset.getObject (2);
     OracleClob clob = (OracleClob) rset.getObject (3);

      System.out.println ("Open the lobs");
      blob.open (LargeObjectAccessMode.MODE_READWRITE);
      clob.open (LargeObjectAccessMode.MODE_READWRITE);
