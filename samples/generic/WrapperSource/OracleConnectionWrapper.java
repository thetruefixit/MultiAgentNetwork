...
:})
  ...
  public void addLogicalTransactionIdEventListener(LogicalTransactionIdEventListener listener)
    throws SQLException
  {
    connection.addLogicalTransactionIdEventListener(listener);
  }
  ...
  public void addLogicalTransactionIdEventListener(LogicalTransactionIdEventListener listener,
    ...
    throws SQLException
  {
    connection.addLogicalTransactionIdEventListener(listener, executor);
  }
  
  public void removeLogicalTransactionIdEventListener(LogicalTransactionIdEventListener listener)
    throws SQLException
  {
    connection.removeLogicalTransactionIdEventListener(listener);
  }
   ...
   * should be purged.
   * @return true - to purge the statement cache
   *         false ? It is ok to use the same statement cache
   * @throws SQLException
   * @since 12.1
   */
 @Override
   ...
   * In case when tagging is used the server will attempt to pair a server
   * process with the same tag.
   *
   * {@link #needToPurgeStatementCache()} returns <tt>true</tt> if the 
   * underlying session has changed and if the local statement cache should 
   * be purged.
   *  </p>
   *
   * @return true - If the tag matched.  In cases without tagging it returns <tt>true</tt> by default
   * @throws  SQLException If there is an exception while obtaining server process
   * @since   12.1
   * @see     #isDRCPEnabled()
   * @see     #detachServerConnection(java.lang.String)
   * @see     #needToPurgeStatementCache()
   */
  @Override
   ...
   * The server can choose to reuse the server process if needed.   The 
   * connection can be released with a tag which esures upon that the next
   * {@link #attachServerConnection()} on this connection the server will
   * attempt to pair this connection with the server process of the same tag.
   *
   * @param tag  A string value that the connection will be associated in the 
   *             the server.   <tt>null</tt> is a valid argument when no tagging
   *             required.
   *
   * @throws  SQLException If there was an exception while releasing
   * @since   12.1
   * @see     #isDRCPEnabled()
   * @see     #attachServerConnection()
   * @see     #needToPurgeStatementCache()
   */
  @Override
  public void detachServerConnection(String tag) throws SQLException {
    connection.detachServerConnection(tag);
  }

  /**
   * Returns if the connection is participating in DRCP.
   * @throws SQLException if there is an error while processing the request
   * @return true if DRCP is enabled
   * @see     #attachServerConnection()
   * @see     #detachServerConnection(java.lang.String)
   * @see     #needToPurgeStatementCache()
   */
...
}// end of  class OracleConnectionWrapper

/*
 * MODIFIED      (MM/DD/YY)
 * ashivaru    07/30/12 - 14133017: return value for attach and detachConnection
 * ashivaru    08/08/11 - Implement DRCP support
 * jdelavar    07/07/11 - add LTXID support
 * scheng      04/15/09 - bug8417308 - implement setApplicationContext and
 *                        clearAllApplicationContext
 * jdelavar    03/05/09 - add getDatabaseTimeZone()
 * ashivaru    05/21/07 - 5952452: add set/getDefaultTimeZone
 * ashivaru    12/15/06 - 5733457: Avoid repeated calling of methods  
 * jdelavar    10/10/06 - migrate new 11gR1 APIs to using enum
 * dsurber     10/11/06 - jdbc 40 final
 * tzhou       08/08/06 - Support connection isUsable
 * dsurber     06/22/06 - jdk6 beta 2 
 * ashivaru    06/22/06 - 5176155: introduce getCurrentSchema
 * jdelavar    06/19/06 - add security feedback info 
 * eshirk      06/13/06 - Add descriptor bulk loading api's 
 * jdelavar    06/08/06 - move cancel() to public interface 
 * dsurber     06/08/06 - make abort public 
 * jdelavar    04/25/06 - new commit options 
 * dsurber     05/02/06 - remove createBFILE 
 * dsurber     03/22/06 - fix syntax error 
 * dsurber     03/21/06 - new factory methods 
 * jdelavar    09/29/05 - add registerAQNotification 
 * jdelavar    09/29/05 - grabtrans 'jdelavar_11g_aq' 
 * dsurber     02/24/06 - update for beta spec 
 * dsurber     01/23/06 - add JDBC 4.0 stub methods 
 * tzhou       09/13/05 - Added pingDatabase() 
 * jdelavar    07/21/05 - add startup/shutdown 
 * jdelavar    08/11/05 - 11gR1 db change notification project 
 * jdelavar    05/27/05 - add aq api 
 * jdelavar    05/20/05 - add AQ 
 * rchen       05/24/04 - push set/get Holdability out for jdk1.2 as well 
 * dsurber     02/27/04 - 
 * jdelavar    11/06/03 -
 * jdelavar    11/04/03 -
 * jdelavar    11/03/03 - add jdbc30 apis
 * dsurber     07/08/03 - update e2e api
 * dsurber     05/30/03 -
 * dsurber     05/07/03 - end to end
 * jdelavar    04/17/03 - empty constructor
 * jdelavar    03/24/03 - add isProxySession()
 * jdelavar    03/21/03 - add openProxySession()
 * lodeng      02/17/03 - Plsql Compiler Warnings
 * rirudaya    10/25/02 - Add implicit connection cache APIs
 * jdelavar    09/07/02 - Introduce m4
 * dsurber     06/17/02 - remove ultra
 * jdelavar    04/15/02 - Change OracleLog activation test
 * surman      12/11/01 - Change getXXXWithKey signatures
 * tzhou       11/08/01 - Change Oracle extension savepoint method names
 * eshirk      11/05/01 - Rollback adding throws clause to getStmtCacheSize
 * surman      10/26/01 - Update for statement caching
 * jdelavar    10/22/01 - Add getProperties()
 * eshirk      10/17/01 - Merged eshirk_connection_wrapping
 * eshirk      10/17/01 - add JDBC 3.0 forwarders
 * eshirk      10/12/01 - Creation
 */

/**
 * @version $Header: dbjava/src/java/oracle/jdbc/OracleConnectionWrapper.java /main/54 2012/09/04 12:44:17 ashivaru Exp $
 * @author  eshirk
 * @since   9.2
 */
